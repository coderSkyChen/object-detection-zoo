...
class MySampler(Sampler):
    def __init__(self, train_size, batch_size):
        ...
        self.batches = int(train_size / batch_size)
        self.batch_size = batch_size
        self.range = torch.arange(0, batch_size).view(1, batch_size).long()
        self.leftover_flag = False
        if train_size % batch_size:
            self.leftover = torch.arange(self.batches * batch_size, train_size).long()
            self.leftover_flag = True

    def __iter__(self): # it's randperm between batches
        rand_num = torch.randperm(self.batches).view(-1, 1) * self.batch_size
        self.rand_num = rand_num.expand(self.batches, self.batch_size) + \
                        self.range
...
if __name__ == '__main__':
    ...
    dataset = RoiBatchLoader(roidb, ratio_list, ratio_index, args.batch_size,\
                             len(pd.classes), args, training=True)

    dataloader = torch.utils.data.DataLoader(dataset, batch_size=args.batch_size, \
                 num_workers=args.num_workers, sampler=sampler_batch)
    ...
    for epoch in range(args.start_epoch, args.max_epochs):
        ...
        for step in range(iters_per_epoch):
            ...
            if step % args.disp_interval == 0:
                ...
                print(
                    "[epoch %2d][iter %4d/%4d] loss: %.4f, lr: %.2e; fg/bg=(%d/%d),time cost: %f; rpn_cls: %.4f, rpn_box: %.4f, rcnn_cls: %.4f, rcnn_box %.4f" % (epoch, step, iters_per_epoch, loss_temp, lr, fg_cnt, \
                       bg_cnt, end - start, loss_rpn_cls,
